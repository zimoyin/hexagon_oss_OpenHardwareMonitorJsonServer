    ...
    internal static class Program {
        public static void Main(string[] args) {
            ...
            if (portArg != null) {
                ...
                if (!string.IsNullOrEmpty(portValue) && int.TryParse(portValue, out var providedPort)) {
                    port = providedPort;
                }
            }
            ...
            if (args.FirstOrDefault(arg => arg.StartsWith("help")) != null || args.FirstOrDefault(arg => arg.StartsWith("-help")) != null) {
                ...
                Console.WriteLine("     Net                            v8.0-windows");
                Console.WriteLine("     OpenHardwareMonitorJsonServer  v2.0.1(Feb 11, 2025)");
                Console.WriteLine("     OpenHardwareMonitor            v0.10.3-alpha1(Jul 4, 2023)");
                Console.WriteLine("     OpenHardwareMonitor-branche    @hexagon-oss/openhardwaremonitor");
                Console.WriteLine("参数：");
                Console.WriteLine("     -p           指定WEB端口并开启服务器(默认8080)");
                Console.WriteLine("     --console    以控制台Json格式返回硬件信息与WEB方式冲突（默认false）");
                Console.WriteLine("     --group      以群组格式返回硬件信息（默认true）");
                Console.WriteLine("     --mainboard  启用主板监控 (默认true)");
                Console.WriteLine("     --cpu        启用CPU监控 (默认true)");
                Console.WriteLine("     --ram        启用内存监控 (默认true)");
                Console.WriteLine("     --gpu        启用GPU监控 (默认true)");
                Console.WriteLine("     --fan        启用风扇控制器监控 (默认true)");
                Console.WriteLine("     --hdd        启用硬盘监控 (默认true)");
                Console.WriteLine("     --network    启用网络监控 (默认true)");
                return;
            }
            ...
            var networkEnabled = GetBoolArgValue(args, "--network", true);

            var isEnabled = networkEnabled || hddEnabled || mainboardEnabled || cpuEnabled || ramEnabled || gpuEnabled || fanControllerEnabled;
            if (!isEnabled) {
                Console.Error.WriteLine("ERROR: Turn on at least one sensor; 至少开启一个传感器");
                return;
            }
        ...
        private static string GetHardware(Computer computer, Visitor visitor, bool isGroup = true) {
            ...
            return isGroup ? JsonConvert.SerializeObject(groupedHardware, settings) : JsonConvert.SerializeObject(computer.Hardware, settings);
        }
